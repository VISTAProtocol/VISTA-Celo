...
export class Vista {
  ...
  showEarningOverlay(params?: EarningOverlayParams): void {
    ...
    const targetEl = params?.targetElement ?? null;
    ...
    let amount = isNaN(status.sessionAmount) ? 0 : status.sessionAmount;
    const score = isNaN(status.score) ? 0 : status.score;
    const secs = isNaN(status.validSeconds) ? 0 : status.validSeconds;

    const fmt = (v: number) =>
      `$${v.toFixed(6)}&thinsp;<span style="font-size:13px;color:${c.accent};font-weight:600;">USDC</span>`;

    const el = document.createElement('div');
    el.id = 'vista-earning-overlay';
    Object.assign(el.style, {
      position: 'fixed', width: `${W}px`, top: '-9999px', left: '-9999px',
      backgroundColor: c.bg, border: `1px solid ${c.border}`, borderRadius: '18px',
      padding: '16px', zIndex: '2147483647',
      fontFamily: 'system-ui, -apple-system, BlinkMacSystemFont, sans-serif',
      color: c.text, backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)',
    ...
    el.innerHTML = `
      ...
      <div style="margin-bottom:13px;">
        ...
        <div id="vista-overlay-amount" style="font-size:24px;font-weight:700;color:${c.text};letter-spacing:-0.02em;line-height:1.1;">${fmt(amount)}</div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:13px;">
        <div style="background:${c.surface};border-radius:10px;padding:9px 11px;">
          ...
          <div id="vista-overlay-score" style="font-size:16px;font-weight:700;">${Math.round(score * 100)}%</div>
        </div>
        <div style="background:${c.surface};border-radius:10px;padding:9px 11px;">
          <div style="font-size:10px;color:${c.muted};text-transform:uppercase;letter-spacing:0.07em;margin-bottom:4px;">Duration</div>
          <div id="vista-overlay-seconds" style="font-size:16px;font-weight:700;">${Math.floor(secs / 60)}:${String(secs % 60).padStart(2, '0')}</div>
        </div>
      </div>
    ...
    const reposition = () => {
      const overlay = document.getElementById('vista-earning-overlay');
      if (!overlay) return;
      if (fsEl && !targetEl) {
      ...
      const anchor = targetEl ?? (fsEl as HTMLElement | null);
      if (!anchor) {
      ...
      const left = spaceRight >= W ? rect.right + GAP : Math.max(4, rect.left - W - GAP);
      overlay.style.top = `${Math.max(4, rect.top + 4)}px`;
    ...
    if (fsEl && !targetEl) {
      (fsEl as HTMLElement).style.position = (fsEl as HTMLElement).style.position || 'relative';
      fsEl.appendChild(el);
    ...
    this.overlayScrollHandler = reposition;
    ...
    this.overlayIntervalId = window.setInterval(() => {
      ...
      const cur = isNaN(s.sessionAmount) ? 0 : s.sessionAmount;
      const sc = isNaN(s.score) ? 0 : s.score;
      ...
      if (amountEl && cur !== prevAmount) {
        const prev = prevAmount;
        prevAmount = cur;
        this.animateValue(amountEl, prev, cur, 700, fmt);
      }
      if (scoreEl) scoreEl.textContent = `${Math.round(sc * 100)}%`;
      if (secondsEl) {
        const dur = Math.floor((Date.now() - this.startTime) / 1000);
        secondsEl.textContent = `${Math.floor(dur / 60)}:${String(dur % 60).padStart(2, '0')}`;
      }
